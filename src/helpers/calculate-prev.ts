...
export function calculatePrevMonth(month: string, year: number): { month: string; date: string; day: string; year: number }[] {
    ...
    const daysInMonth: number = new Date(prevMonthDate.getFullYear(), prevMonthDate.getMonth()+1, 0).getDate();
    const firstDayOfMonth: number = new Date(prevMonthDate.getFullYear(), prevMonthDate.getMonth()+1, 1).getDay();
